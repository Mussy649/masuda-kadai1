...
- `layouts/app.blade.php` を作成し、共通ヘッダー・共通レイアウトを整理しました。
- `common.css` に集約していたCSSを、画面ごとに以下へ分割しました。
  - `index.css`
  - `confirm.css`
  - `thanks.css`
  - `admin.css`
  - `auth.css`
- thanks画面は課題UIに合わせ、共通ヘッダーを使わず独立した画面構成にしています。

### 追加機能

- 入力画面に「どこで知りましたか？」のチェックボックスを追加しました。
  - 複数選択可能
  - 未選択時は日本語バリデーションメッセージを表示
  - 確認画面・管理画面詳細モーダルに選択内容を表示
- 入力画面に画像アップロード想定の項目を追加しました。
  - 今回は模擬案件用として、用意済みのダミー画像から選択する方式で実装
  - 確認画面・管理画面詳細モーダルに選択画像を表示
  - DBには画像パスを `image_path` として保存

### DB追加カラム

`contacts` テーブルに以下のカラムを追加しました。
